...
> Operator note: keep this report only as an archive of the old validation flow. The supported runtime path today is `telegram-events` + `events-engine` + `secretary`, with outbound Telegram sends handled by `better-telegram-mcp`.
...
```
**Статус:** Historical evidence only — the standalone container worked at the time of this report, but the service is no longer part of the active stack and has been replaced by the `better-telegram-mcp` relay.
...
Standalone telegram-mcp контейнер успешно отправил сообщение (historical; this path was later removed from the active stack and replaced by the `better-telegram-mcp` relay):
- Message ID: 5522
...
✅ **telegram-mcp** - historical standalone sender, now removed from the active stack
✅ **better-telegram-mcp** - current relay for outbound Telegram sends

## Что НЕ работает

❌ **Agent-mux dispatch** - события не обрабатываются автоматически
❌ **Secretary автоответы** - не может автоматически отвечать на Telegram сообщения without the relay path
...
2. **Отправка ответов:** Use the `better-telegram-mcp` relay instead of the removed standalone `telegram-mcp` container ✅

Пример:
```bash
# Secretary получил событие (видно в last_status)
# Legacy example only: do not run against the removed telegram-mcp service.
# Use the better-telegram-mcp relay for outbound sending in current deployments.
```
