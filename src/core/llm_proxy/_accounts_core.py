...
def _load_json(path: Path, *, default: dict[str, Any]) -> dict[str, Any]:
    if not path.exists():
        return cast(dict[str, Any], json.loads(json.dumps(default)))
...
def prune_runtime_state(
    ...
    payload = cast(dict[str, Any], json.loads(json.dumps(runtime_state)))
    active = set(active_profile_ids)
    profiles = payload.get("profiles")
    if not isinstance(profiles, dict):
        payload["profiles"] = {}
        return payload
    payload["profiles"] = cast(
        dict[str, Any],
        {
            profile_id: state
    ...
    *,
    ...
    for row in ordered_rows:
        ...
        if not isinstance(state, dict):
            state = {}
        probe_http_status: Any = state.get("last_probe_http_status")
        probe_latency_ms: Any = state.get("last_probe_latency_ms")
        ...
        results.append(
            {
                **row,
                ...
                "last_probe_http_status": (
                    int(probe_http_status) if isinstance(probe_http_status, int) else None
                ),
                "last_probe_latency_ms": (
                    int(probe_latency_ms) if isinstance(probe_latency_ms, int) else None
                ),
