...
def _assert_before_and_cleared(
    ...
    for actual, expected in (
        (before_details[1]["context_id"], before_context_id),
        (before_details[2]["recent_entries"], []),
        (cleared_details[1]["previous_context_id"], before_context_id),
        (cleared_details[1]["context_id"], cleared_context_id),
        (cleared_details[2]["context_id"], cleared_context_id),
        (cleared_details[2]["recent_entries"], []),
    ):
...
def _assert_restarted(
    ...
    for actual, expected in (
        ...
        (restarted_details[2]["context_id"], cleared_details[0]),
        (restarted_details[2]["previous_context_id"], before_details[0]),
        (restarted_details[2]["recent_entries"], []),
    ):
        test_case.assertEqual(actual, expected)
