...
from core.orchestra_agents.templates.opencode.agent_runtime.backend_context import clear_file
from core.orchestra_agents.templates.opencode.agent_runtime.backend_status import (
    build_status_extras,
)
...
class OpencodeOmoBackend(BaseAgentBackend):
    ...
    async def stop(self, request: StopRequest) -> dict[str, Any]:
        if backend_dispatch.dispatch_matches(
            self._dispatch, request.thread_id, request.parent_thread_id
        ):
            await backend_dispatch.cancel_dispatch(self._dispatch)
