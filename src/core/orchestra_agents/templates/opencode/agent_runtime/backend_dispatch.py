...
from core.orchestra_agents.templates.opencode.agent_runtime.backend_context import (
    clear_file,
...
async def run_dispatch(backend: OpencodeOmoBackend, event: AgentEvent) -> None:
    ...
    async with backend._dispatch.lock:
        write_active_context(
            backend._paths.active_context, event, backend.context.current_id, backend.agent_slug
        )
        try:
