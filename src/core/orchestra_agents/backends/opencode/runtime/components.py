...
from core.orchestra_agents.backends.opencode.event_dispatch import EventDispatcher
...
from core.orchestra_agents.backends.opencode.session_manager import SessionManager
...
async def shutdown_components(components: Components) -> None:
    if components.client is not None:
        await close_client(components.client)
