...
class AgentLifecycleE2ETests(BaseE2ETestCase):
    ...
    async def test_agent_status_reports_busy(self) -> None:
        agents = await add_pair(self.harness)
...
async def _run_busy_status_flow(
    ...
    status_payload = await harness.get_agent_status(agents["orchestra"].slug)
    ...
    status_after = await harness.get_agent_status(agents["orchestra"].slug)
    case.assertEqual(status_after["status"], "idle")
    case.assertIsNone(status_after["current_thread_id"])
