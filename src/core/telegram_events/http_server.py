...
async def healthz(request: web.Request) -> web.Response:
    ...
    return web.json_response({"ok": True, "service": "telegram-events"})
...
async def send(request: web.Request) -> web.Response:
    ...
    if client is None:
        return web.json_response(
            {"ok": False, "error": "Telethon client not initialized"},
            status=503,
        )
    try:
...
async def _handle_send(client: Any, body: dict[str, Any]) -> web.Response:
    ...
    if not isinstance(chat_id, int):
        return web.json_response({"ok": False, "error": "chat_id must be an integer"}, status=400)
...
async def _execute_send(client: Any, chat_id: int, message: str) -> web.Response:
    try:
        ...
        return web.json_response(
            {"ok": False, "error": f"Failed to resolve chat: {exc}"},
            status=502,
        )
    try:
        ...
        return web.json_response(
            {"ok": False, "error": f"Failed to send message: {exc}"},
            status=502,
        )
